  ...
  *varargout_1_size = varargin_1_size;
  for (ret = 0; ret < varargin_1_size; ret++) {
    ...
    if (y_im == 0.0) {
      ...
      theta = std::exp(y_re / 2.0);
      y_re = theta * (theta * std::cos(y_im));
      y_im = theta * (theta * std::sin(y_im));
    }
    previousSample_re =
        varargin_1_data[ret].re * y_re - varargin_1_data[ret].im * y_im;
    previousSample_im =
        varargin_1_data[ret].re * y_im + varargin_1_data[ret].im * y_re;
    varargout_1_data[ret].re = previousSample_re;
    varargout_1_data[ret].im = previousSample_im;
  ...
  for (ret = 0; ret < varargin_1_size; ret++) {
    theta = varargout_1_data[ret].re;
    d = varargout_1_data[ret].im;
    PhaseRecoveryGain = theta * y_im + d * y_re;
    theta = theta * y_re - d * y_im;
    varargout_1_data[ret].re = theta;
    varargout_1_data[ret].im = PhaseRecoveryGain;
  }
