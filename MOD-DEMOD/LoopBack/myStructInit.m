function [SimParams] = myStructInit(message)
coder.inline('always');
...
messageLength             = coder.const(strlength(message));               % 'Message'       
numberOfMessages          = coder.const(1);
...
plutoCenterFrequency      = coder.const(400000000);
plutoGain                 = 30;
plutoFrontEndSampleRate   = coder.const(fs);
plutoFrameLength          = coder.const(interpolation * frameSize);
address                   = coder.const('ip:192.168.2.1') ; % Specify Radio ID

SimParams.PlutoCenterFrequency      = coder.const(plutoCenterFrequency);
SimParams.PlutoGain                 = coder.const(plutoGain);
SimParams.PlutoFrontEndSampleRate   = coder.const(plutoFrontEndSampleRate);
SimParams.PlutoFrameLength          = coder.const(plutoFrameLength);
SimParams.Address                   = coder.const(address) ; % Specify Radio ID
