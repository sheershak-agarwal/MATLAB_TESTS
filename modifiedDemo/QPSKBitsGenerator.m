...
%        MessageBits = [];
    ...
    methods (Access=protected)
        function setupImpl(obj, ~)
            ...
            obj.pSigSrc = dsp.AsyncBuffer(length(obj.MessageBits));
            write(obj.pSigSrc, obj.MessageBits);
...
%                 'SignalEndAction', 'Cyclic repetition');
        ...
        function [y, msgBin] = stepImpl(obj)
            ...
            len = obj.MessageLength * 7 * obj.NumberOfMessage;
            if (obj.pSigSrc.NumUnreadSamples >= len)
               msgBin = read(obj.pSigSrc, len);
              % Scramble the data
